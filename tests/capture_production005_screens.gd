...
func _init() -> void:
	...
	if _output_dir.is_empty():
		push_error("capture_production005_screens.gd requires --output-dir <absolute_or_relative_path>")
		quit(1)
		return
	...
	await _boot_main()
	await _capture_town_plaza()
	await _capture_home()
	await _capture_shop()
	await _capture_school_gate()
	await _capture_school_yard()
...
func _boot_main() -> void:
	...
	_main.configure_for_test("user://production005_capture_save_%s.json" % _suffix)
...
func _seed_base_state() -> void:
	...
	_main.call("_update_loop_status", "小镇准备好")
	_main.call("_update_today_status")


func _capture_town_plaza() -> void:
	_main.show_town_view()
	_main.move_player_to_cell(Vector2i(14, 10))
	_main.interact_nearby()
	await _settle_frames()
	_save_viewport_image("shot_production005_town_plaza_%s.png" % _suffix)


func _capture_home() -> void:
	_main.show_home_view()
	await _settle_frames()
	_save_viewport_image("shot_production005_home_%s.png" % _suffix)
...
func _capture_shop() -> void:
	...
	_main.interact_nearby()
	await _settle_frames()
	_save_viewport_image("shot_production005_shop_%s.png" % _suffix)


func _capture_school_gate() -> void:
	_main.show_town_view()
	_main.set_day_key_for_test("local_day_006")
	_main.call("_update_today_status")
	...
	_main.interact_nearby()
	await _settle_frames()
	_save_viewport_image("shot_production005_school_gate_%s.png" % _suffix)


func _capture_school_yard() -> void:
	_main.show_town_view()
	_main.set_day_key_for_test("local_day_006")
	_main.call("_update_today_status")
	...
	_main.interact_nearby()
	await _settle_frames()
	_save_viewport_image("shot_production005_school_yard_%s.png" % _suffix)


func _capture_album() -> void:
	_main.open_memory_album()
	await _settle_frames()
	_save_viewport_image("shot_production005_album_%s.png" % _suffix)
	_main.close_memory_album()


func _capture_settings() -> void:
	_main.open_settings_panel()
	await _settle_frames()
	_save_viewport_image("shot_production005_settings_%s.png" % _suffix)
...
func _save_viewport_image(file_name: String) -> void:
	...
	if viewport_texture == null:
		push_error("PRODUCTION005_CAPTURE_BLOCKED: headless dummy renderer does not expose a viewport texture. Use MCP or a non-headless display path for screenshot evidence.")
		quit(2)
		return
	var image: Image = viewport_texture.get_image()
	if image == null:
		push_error("PRODUCTION005_CAPTURE_BLOCKED: viewport image is unavailable under the current renderer. Use MCP or a non-headless display path for screenshot evidence.")
		quit(2)
		return
