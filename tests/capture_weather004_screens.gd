...
func _init() -> void:
	...
	if _output_dir.is_empty():
		push_error("capture_weather004_screens.gd requires --output-dir <absolute_or_relative_path>")
		quit(1)
		return
	...
	await _boot_main()
	await _capture_sunny_sun()
	await _capture_breezy_kite()
	await _capture_light_rain_bear()
	await _capture_after_rain_umbrella_album()
...
func _boot_main() -> void:
	...
	_main.configure_for_test("user://weather004_capture_save_%s.json" % _suffix)
...
func _capture_sunny_sun() -> void:
	_main.show_town_view()
	_main.set_day_key_for_test("local_day_001")
	_main.call("_update_today_status")
	_main.move_player_to_cell(Vector2i(17, 2))
	_main.interact_nearby()
	await _settle_frames()
	_save_viewport_image("shot_weather004_sunny_sun_%s.png" % _suffix)


func _capture_breezy_kite() -> void:
	_main.show_town_view()
	_main.set_day_key_for_test("local_day_002")
	_main.call("_update_today_status")
	_main.move_player_to_cell(Vector2i(16, 4))
	_main.interact_nearby()
	await _settle_frames()
	_save_viewport_image("shot_weather004_breezy_kite_%s.png" % _suffix)


func _capture_light_rain_bear() -> void:
	_main.show_town_view()
	...
	_main.interact_nearby()
	await _settle_frames()
	_save_viewport_image("shot_weather004_light_rain_bear_%s.png" % _suffix)


func _capture_after_rain_umbrella_album() -> void:
	_main.show_town_view()
	_main.set_day_key_for_test("local_day_006")
	_main.call("_update_today_status")
	_main.move_player_to_cell(Vector2i(32, 9))
	_main.interact_nearby()
	_main.open_memory_album()
	await _settle_frames()
	_save_viewport_image("shot_weather004_after_rain_album_%s.png" % _suffix)
	_main.close_memory_album()
...
func _capture_shop_home_safe_path() -> void:
	...
	_main.interact_nearby()
	await _settle_frames()
	_save_viewport_image("shot_weather004_shop_%s.png" % _suffix)
	_main.show_home_view()
	await _settle_frames()
	_save_viewport_image("shot_weather004_home_%s.png" % _suffix)
...
func _save_viewport_image(file_name: String) -> void:
	...
	if viewport_texture == null:
		push_error("WEATHER004_CAPTURE_BLOCKED: headless dummy renderer does not expose a viewport texture. Use MCP or a non-headless display path for screenshot evidence.")
		quit(2)
		return
	var image: Image = viewport_texture.get_image()
	if image == null:
		push_error("WEATHER004_CAPTURE_BLOCKED: viewport image is unavailable under the current renderer. Use MCP or a non-headless display path for screenshot evidence.")
		quit(2)
		return
