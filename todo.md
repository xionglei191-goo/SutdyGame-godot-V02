...
> 当前里程碑：Round183 执行 visual rebuild 资产运行时优先替换；V02.39 mainline whitebox 已进入 bitmap promotion 返修轮
...
| 项目 | 当前状态 |
|---|---|
| 当前轮次 | Round183：V02-VISUALREPLACE-001 visual rebuild 资产运行时优先替换已完成 |
| 本轮目标 | 已按用户最新指令，将首批 Round169-Round179 visual rebuild 候选资产接入真实 runtime；这些资产不再仅作为 proof-only 证据，后续按运行截图逐项返修不合格资产；未读改 `docs/问答纪要.md` |
| 进行中 | 暂无 |
| Ready | 暂无 |
...
| 阻塞项 | 暂无 V02.39 / StoryBatch 主线硬阻塞 |
| 待确认决策 | 无；已采用 V02.39 Godot 视觉生产体系重建路线，full-map background 禁用仍为最终目标 |
| 临时默认值 | V02.37 在 Round153 后暂停；Round153 第二批 production 资产保留为库存但不进入 final approval；`docs/collaboration/artpass003_visual_direction/`、该目录 `README.md` 和 Round92 是当前视觉硬门槛；该目录只放 PM / Art Direction 认可的正向 Animal Crossing-like cozy town 参考，不得放入用户已驳回的错误截图、拼贴图、逻辑地图截图或临时 target 失败图；`place.world_map.base_1280` / `world_map_base_1280.png` 只能作为 reference，不得作为 final runtime 主画面；状态语义为 `production`、`functional_pass`、`visual_scaffold`、`art_target_locked`、`runtime_visual_match`、`runtime_promoted_for_review`、`final_approved`；Round119 历史 `approved` 改按旧阶段 `functional_pass` 管理；Round159 `visual_candidate` 项目级重解释为 `visual_scaffold`；A-Z 默认 prop-first，字母 badge 只作辅助；V02.39 必须先 target 1280 frame，再 visual layout contract，再 runtime match；V02.39 generated scene-like PNG patch 只保留为 failed sample，不得重新接入 runtime resolver，除非先通过 tileability / transparency / scale / runtime screenshot 复核；Round167 img2img sheet 只作 style / crop 候选证据，不得映射 ThemeProfile / AssetResolver；按 2026-06-08 用户最新指令，Round169-Round179 visual rebuild atlases / normalized PNGs 可先映射 ThemeProfile / AssetResolver 进入 runtime，状态标记为 `runtime_promoted_for_review`，再用 1280 runtime screenshot / gate 逐项返修；UI glass 不得默认用白底清底，需 chroma-key 或真透明路径；底层保留 cell / footprint / occupied cells，孩子端不显示格子、瓦块、坐标或调试术语；Apple-like translucent glass UI；不采用绘本 / storybook / picture-book / 课程面板 / 厚卡片 / 裸字母牌 / 系统索引地图 / 功能点拼贴作为正式方向；素材必须有 logical asset ID；当前未完成视觉验收只看 1280x720，960x540 等全部开发完成后再做版本适配；A-Z 锚点常驻地图；Letter Snake 仅可选；账号/云存档/语音/AI/社交均为本地 stub |
...
| Godot / Narrative / QA | V02-STORYBATCH-004 第二批 runtime 接入与真实入口 smoke | 完成 | Round181 gate 解除阻塞；既有 `tests/test_v0237_storybatch004_runtime_smoke.gd` 通过，11 个 story props 真实入口 / A-Z 落账 / 儿童安全均保持 |
| QA / PM / Art Direction | V02-STORYBATCH-005 1280 proof、approved 判定与阶段收口 | 完成 | Round182 已补齐 7 张第二批 1280 proof、approval gate JSON、focused gate 和 full runner；第二批为 `runtime_visual_match`，final bitmap art approval 递延 |
...
| PM / Art Direction / Tech Art / UX | V02-VISUALREBUILD-002 1280 目标画面与 Godot 可执行视觉合同 | 完成 | Round181 已将 Round180 Godot 白盒目标推进为 `art_target_locked_whitebox_by_user_instruction`；`visual_layout_contract_v001.json`、1280 target PNG、capture、focused layout test 和 full runner 均通过；锁定范围为白盒构图 / runtime match，不等于最终 bitmap art approval |
| Tech Lead / Godot / Tooling / QA | V02-VISUALREBUILD-003 Logic Map / Visual Layout 分层方案 | 完成 | Round162 已完成 Logic Map / Visual Layout 边界、visual anchor / depth / proxy / occlusion / focus state 合同和迁移策略 |
| Art Direction / Asset / Tech Art | V02-VISUALREBUILD-004 首屏统一环境资产包与 composition kit | 完成 | Round181 新增 `unified_environment_composition_kit_v001.json`，以 logical whitebox asset IDs、runtime texture keys、layer / scale / pivot / role 约束锁定 composition kit；不接 ThemeProfile / AssetResolver 新映射 |
| Godot / Tech Art / QA | V02-VISUALREBUILD-005 TownStage runtime visual match 纵切 | 完成 | Round181 新增 `runtime_visual_match_report_v001.json`、side-by-side 1280 proof 和 focused gate；`TownStage.get_visual_rebuild_blockout_snapshot()`、真实 Home 入口、五按钮 footer、A-Z 稳定均通过 |
| PM / Art Direction / UX / QA | V02-VISUALREBUILD-006 Art-direction gate 与 StoryBatch 解锁判定 | 完成 | Round181 新增 `art_direction_gate_v001.json`，判定 `runtime_visual_match` whitebox pass，解除 StoryBatch-004 阻塞；不声明 final bitmap art approval |
| Tech Art / Godot / QA | V02-VISUALREPLACE-001 visual rebuild 资产运行时优先替换 | 完成 | Round183 已将 125 个 visual rebuild logical asset runtime mappings 接入 ThemeProfile / AssetResolver，覆盖第一屏 `v0239_*` 与现有 place / character / furniture / UI / anchor / story prop 映射；focused / full runner、Godot 启动、diff check 和持久 1280 proof 导出通过 |
...
| [x] | V02-VISUALREBUILD-002 | PM / Art Direction / Tech Art / UX | 1280 target gameplay frame、Godot 可执行视觉合同 | Round181 已将 Round180 Godot 白盒目标推进为 `art_target_locked_whitebox_by_user_instruction`，并通过 layout / blockout / mainline gate / full runner 验证；锁定范围为 whitebox composition / runtime static-frame match，不等于最终 bitmap art approval |
| [x] | V02-VISUALREBUILD-003 | Tech Lead / Godot / Tooling / QA | Logic Map / Visual Layout 分层方案 | `world_map` 继续保留 cell / collision / interaction / A-Z ID / save 事实；Visual Layout 独立管理 visual anchors / depth / proxy / occlusion / focus state |
| [x] | V02-VISUALREBUILD-004 | Art Direction / Asset / Tech Art | 首屏统一环境资产包与 composition kit | Round181 `unified_environment_composition_kit_v001.json` 已锁定 whitebox logical asset IDs、runtime texture keys、layer / scale / pivot / role 约束；不混入旧 scene fragment |
| [x] | V02-VISUALREBUILD-005 | Godot / Tech Art / QA | TownStage runtime visual match 纵切 | Round181 runtime match report、side-by-side 1280 proof、focused mainline gate 和 full runner 通过；真实入口不退化 |
| [x] | V02-VISUALREBUILD-006 | PM / Art Direction / UX / QA | Art-direction gate 与 StoryBatch 解锁判定 | Round181 gate 通过：`runtime_visual_match` whitebox pass，A-Z 稳定、儿童安全和真实入口通过，StoryBatch-004 阻塞解除 |
| [x] | V02-VISUALREPLACE-001 | Tech Art / Godot / QA | Round169-Round179 visual rebuild PNG runtime promotion、ThemeProfile / AssetResolver 映射、TownStage 第一屏替换、focused / full runner 和 runtime proof | Round183 已接入 125 个 visual rebuild runtime resolver mappings，并保留 160 条 `runtime_promoted_for_review` / `runtime_promoted_pending_visual_review` acceptance 记录；游戏第一屏 `v0239_*` keys 优先使用 PNG，现有 place / character / furniture / UI / anchor / story prop 映射也优先走 visual rebuild 候选；A-Z anchor / Home 真实入口 / 五按钮 footer 不退化；不合格资产进入后续运行截图返修 |
...
- [x] **V02-STORYBATCH-004 第二批 runtime 接入与真实入口 smoke**
  Owner：Godot / Narrative / QA Agent；依赖：V02-STORYBATCH-003、V02-VISUALREBUILD-006；交付物：第二批 story prop runtime 渲染、`look_story_prop` 或等价真实入口、story slice save record、A-Z card state 落账和 focused smoke；验收：V02.39 Round181 已达到 `runtime_visual_match` whitebox pass，阻塞解除；既有 `tests/test_v0237_storybatch004_runtime_smoke.gd` 通过，11 个 story props 真实入口、A-Z 落账和儿童安全均保持。完成记录：Round181 gate 将本任务从阻塞转为完成；不把 proof-only asset packs 自动升级 final art。

- [x] **V02-STORYBATCH-005 1280 proof、approved 判定与阶段收口**
  Owner：QA / PM / Art Direction Agent；依赖：V02-STORYBATCH-002、V02-STORYBATCH-003、V02-STORYBATCH-004、V02-VISUALREBUILD-006；交付物：focused / full runner、JSON、diff check、Godot 启动、1280 runtime proof、逐项 `production` / `functional_pass` / `visual_scaffold` / `runtime_visual_match` / `final_approved` / `needs_fix` 判定和阶段收口记录；完成记录：Round182 新增 `docs/collaboration/round182_storybatch005_rc/storybatch005_approval_gate_v001.json`、7 张第二批 1280 proof、`tests/capture_round182_storybatch005_rc.gd`、`tests/test_v0237_storybatch005_approval_gate.gd` 和 `docs/collaboration/Round182_V02.37_STORYBATCH-005_1280ProofApprovedDecisionCloseout.md`；第二批 story props 判定为 `runtime_visual_match`，ThemeProfile acceptance 仍保持 `production` 并指向 Round182 viewport evidence；第一批维持 `functional_pass`；final bitmap art approval 递延，proof-only asset packs 不升级。
...
- [x] **V02-VISUALREBUILD-002 1280 目标画面与 Godot 可执行视觉合同**
  Owner：PM / Art Direction / Tech Art / UX Agent；依赖：V02-VISUALREBUILD-001、artpass003 视觉方向、Round92；交付物：1280 target gameplay frame、paintover 或等价目标图，Godot 可执行 visual layout 合同；验收：home-centered 构图、camera、depth bands、layer order、asset scale、negative space、UI safe area、A-Z 首屏优先级、forbidden remnants 和 proof checklist 明确，达到 `art_target_locked`。返修记录：`docs/collaboration/round161_visual_rebuild_target/target_frame_v0239_round161_home_centered_1280.png` 已被用户复核为完全不是项目想要的游戏画面；该图只保留为 rejected evidence，不得作为资产包生产依据；用户已明确 `docs/collaboration/artpass003_visual_direction/artpass003_main_gameplay_direction_1280.png` 是正确正向参考，返修应按该图的动物森友会式 cozy town 方向转译为 Sunshine Town 可执行 gameplay target frame。Round166 返修补充：首批 retarget 候选均未通过，新增 `docs/collaboration/Round166_V02.39_VISUALREBUILD-002_TileEditableVisualSpec.md`。Round180 主线重置：停止继续扩 proof-only asset warehouse，新增 `docs/collaboration/round180_v0239_mainline_visual_layout_target/visual_layout_contract_v001.json`、`round180_v0239_visual_layout_target_1280x720_v001.png`、`tests/capture_round180_v0239_visual_layout_target.gd` 和 `tests/test_v0239_visual_layout_target.gd`，并接入 `tests/headless_runner.gd`。Round181 完成记录：按用户“完成剩下所有任务”指令，将 Round180 Godot 白盒目标推进为 `art_target_locked_whitebox_by_user_instruction`；锁定范围为 whitebox composition / runtime static-frame match，不等于最终 bitmap art approval。

- [x] **V02-VISUALREBUILD-003 Logic Map / Visual Layout 分层方案**
  Owner：Tech Lead / Godot / Tooling / QA Agent；依赖：V02-VISUALREBUILD-002；交付物：Logic Map / Visual Layout 边界、visual anchor / depth / proxy / occlusion / focus state 合同和迁移策略；验收：`world_map` 继续管 cell / collision / interaction / A-Z ID / save，首屏视觉布局不再由 cell 全量直推。完成记录：新增 `docs/collaboration/Round162_V02.39_VISUALREBUILD-003_LogicMapVisualLayoutPlan.md`；审计当前 `world_map` / `TownStage` 混合驱动画面的问题，固定 Logic Map 继续保留 cell / collision / interaction / A-Z / save 事实，Visual Layout 独立管理 visual anchors、depth、occlusion、proxy 和 focus state；提出 layout JSON / Resource 字段、logical asset ID、fallback、静态 1280 match 优先和真实入口 / A-Z / no-full-map 防回归测试策略；本轮为分层方案，不改 runtime / data / tests / assets。

- [x] **V02-VISUALREBUILD-004 首屏统一环境资产包与 composition kit**
  Owner：Art Direction / Asset / Tech Art Agent；依赖：V02-VISUALREBUILD-002、V02-VISUALREBUILD-003；交付物：terrain、region、building、prop、shadow、actor scale、glass UI 统一资产包；验收：同相机、同光源、同比例、同透视，logical asset ID 完整，不混入旧 scene fragment。完成记录：Round181 新增 `docs/collaboration/round181_v0239_mainline_closeout/unified_environment_composition_kit_v001.json`，锁定 whitebox logical asset IDs、runtime texture keys、layer / scale / pivot / role 约束；不接 ThemeProfile / AssetResolver 新映射。

- [x] **V02-VISUALREBUILD-005 TownStage runtime visual match 纵切**
  Owner：Godot / Tech Art / QA Agent；依赖：V02-VISUALREBUILD-003、V02-VISUALREBUILD-004；交付物：Godot 1280 runtime frame、visual layout binding、必要交互代理和并排 proof；验收：runtime screenshot 与 target frame 并排复核达到 `runtime_visual_match`，真实入口不退化。完成记录：Round181 新增 `runtime_visual_match_report_v001.json`、`round181_v0239_runtime_match_side_by_side_1280x720_v001.png`、`tests/capture_round181_v0239_runtime_match_gate.gd` 和 `tests/test_v0239_visual_rebuild_mainline_gate.gd`；真实 Home 入口、五按钮 footer、26 A-Z anchor、legacy layer hidden 和 no generated-PNG runtime binding 均通过。

- [x] **V02-VISUALREBUILD-006 Art-direction gate 与 StoryBatch 解锁判定**
  Owner：PM / Art Direction / UX / QA Agent；依赖：V02-VISUALREBUILD-005；交付物：final gate 记录、截图包、阻塞解除或继续返修结论；验收：只有 `runtime_visual_match`、真实入口、A-Z 稳定、儿童安全文本和 PM / Art Direction 通过后，`V02-STORYBATCH-004` 才能转 Ready。完成记录：Round181 新增 `art_direction_gate_v001.json`，判定 `runtime_visual_match` whitebox pass，解除 StoryBatch-004 阻塞；`V02-STORYBATCH-005` 转 Ready；不声明 final bitmap art approval。

- [x] **V02-VISUALREPLACE-001 visual rebuild 资产运行时优先替换**
  Owner：Tech Art / Godot / QA Agent；依赖：用户 2026-06-08 最新指令、V02-VISUALREBUILD-005 / 006、Round169-Round179 visual rebuild 候选资产；交付物：ThemeProfile / AssetResolver runtime mappings、TownStage 第一屏 `v0239_*` PNG promotion、focused / full runner、Godot 启动、diff check 和 live viewport evidence；验收：先把候选资产替换进游戏，不再以 proof-only 边界阻止 runtime；游戏第一屏 `v0239_*` keys 优先使用 PNG；A-Z anchor、Home 真实入口、五按钮 footer 和儿童安全不退化。完成记录：Round183 接入 125 个 visual rebuild runtime resolver mappings，覆盖第一屏 `v0239_*` 以及现有 place / character / furniture / UI / anchor / story prop 映射，并保留 160 条 promoted acceptance 记录；`scripts/main.gd` 将 `v0239_*` texture keys 绑定到 ThemeProfile / AssetResolver；`tests/test_v0239_visual_rebuild_blockout.gd`、`tests/test_v0239_visual_rebuild_mainline_gate.gd`、`tests/test_asset_resolver.gd` 和 `tests/headless_runner.gd` 已改为断言 runtime-promoted mapping 与 acceptance path 一致；新增 `docs/collaboration/Round183_V02.39_VISUALREPLACE-001_RuntimeBitmapPromotion.md`；`docs/collaboration/round181_v0239_mainline_closeout/round181_v0239_runtime_match_side_by_side_1280x720_v001.png` 已重新导出为持久 proof；验证命令均通过。
...
| 2026-06-07 | Round 161 返修 / canonical reference retarget | V02-VISUALREBUILD-002 1280 目标画面与 Godot 可执行视觉合同返修 | 组织 PM / Art Direction / Tech Art / UX 分组新增 `docs/collaboration/Round161_V02.39_VISUALREBUILD-002_1280TargetFrameVisualContract.md`；按仓库 image generation fallback 生成 `docs/collaboration/round161_visual_rebuild_target/target_frame_v0239_round161_home_centered_1280.png`，`file` 确认为 1280x720 PNG；用户随后复核确认该图完全不是项目想要的游戏画面，已将该 PNG 改为 rejected evidence；撤销 `art_target_locked`，`V02-VISUALREBUILD-002` 回到 `[~]`，`V02-VISUALREBUILD-004` 不得据此生产资产包；用户已明确 `docs/collaboration/artpass003_visual_direction/artpass003_main_gameplay_direction_1280.png` 是正确正向参考，后续返修应将该 Animal Crossing-like cozy town 方向转译为 Sunshine Town 可执行 gameplay target，而不是照抄、切图或自由重生图；本轮不授予 `runtime_visual_match` / `final_approved`，不解锁 StoryBatch；新增 `LESSON-017` |
| 2026-06-07 | Round 162 验收 / V02.39 继续阻塞资产生产 | V02-VISUALREBUILD-003 Logic Map / Visual Layout 分层方案收口 | 组织 Tech Lead / Godot / Tooling / QA 分组新增 `docs/collaboration/Round162_V02.39_VISUALREBUILD-003_LogicMapVisualLayoutPlan.md`；方案审计当前 `world_map` / `TownStage` 仍由逻辑 cell、place、anchor、NPC 与硬编码视觉片段混合驱动画面，固定 Logic Map 继续保留 cell / collision / interaction / A-Z / save 事实，Visual Layout 独立管理 visual anchors、depth、occlusion、proxy 和 focus state；提出 layout JSON / Resource 字段、logical asset ID、fallback、静态 1280 visual match 优先、真实入口代理恢复和 no-full-map / A-Z / layer ordering / non-prefab marker 降噪测试策略；Round163 资产 composition kit 规格与 Round165 gate 矩阵已作为后续预研落地，但不等同资产生产、runtime match 或 final gate 完成；当前无 Ready，必须等待 `V02-VISUALREBUILD-002` 重做并重新通过 target frame 复核；验证：`git diff --check`、JSON `jq empty`、`godot --headless --path . --quit` 和 `tests/headless_runner.gd` 通过 |
| 2026-06-07 | Round 166 返修规格 / tile-editable visual spec | V02-VISUALREBUILD-002 瓦块可编辑首屏规格 | 基于用户复核结论停止自由生图式 target 试错：Round166 生成候选中 1024 方图尺寸错误，1280x720 候选偏类 3D 且房子过大，`flat_2d_small_home` 仍难支撑瓦块地图编辑，手工 tile mock 又偏离动森质感且文件已删除；新增 `docs/collaboration/Round166_V02.39_VISUALREBUILD-002_TileEditableVisualSpec.md`，固定 Animal Crossing-like 方向要先翻译为 tile / prefab / prop / UI 可编辑规则：1 logic cell ≈ 32px screen、4x4 chunk、Home 5x4 footprint、road 2-3 cells、actor 1x1、prop-first A-Z、glass UI；本轮不授予 `art_target_locked`，不解锁资产包生产，下一步应按该规格做 Godot whitebox / spec-driven target |
| 2026-06-08 | Round 167 图生图 style-reference 测试 | V02-VISUALREBUILD-002 img2img sheet 生产路径验证 | 按用户指定 `/home/xionglei/.codex/generated_images/019ea1f9-af6a-74a1-95f8-8ba7a1f4db32/ig_0d7553c9c33ca64f016a257d184bb8819dbd188a1d01cc3df4.png` 作为 style reference，使用 `/home/xionglei/GameProject/tools/image_generator.js edit` 生成 `docs/collaboration/round167_visual_rebuild_img2img_style_test/round167_img2img_terrain_tile_sheet_v001.png`、`round167_img2img_environment_prop_sheet_v001.png`、`round167_img2img_ui_icon_sheet_v001.png`；三张均为 1024x1024 RGB，并生成 RGBA 清底候选和 `round167_img2img_manifest.json` / Round167 报告；结论：terrain / props 可作为 style / crop 候选，environment props 最接近可人工 crop；UI 原图风格可读但白底清底破坏 glass 内部，需 chroma-key 或真透明路径；本轮不接 runtime、不改 ThemeProfile / AssetResolver、不授予 `production` / `art_target_locked`；新增 `LESSON-019` |
| 2026-06-08 | Round 169 path / fence / flower / home pipeline | V02-VISUALREBUILD-002 normalized_candidate 资产 proof 与 Godot 1280 preview | 使用 `/home/xionglei/GameProject/tools/image_generator.js` fresh same-batch 生成 path / fence / flower / home raw sheets；新增 `scripts/tools/normalize_chroma_path_tile_sheet.py` 并扩展 `scripts/tools/normalize_chroma_grid_sheet.py` 的 visible key cleanup / category component gate；落地 `assets/art/visual_rebuild/round169/{path,fence,flower,home}/` atlas、proof、manifest、README，全部 gate 为 `pass`；新增 `tests/capture_round169_path_fence_flower_home_preview.gd` 并导出 `docs/collaboration/round169_visual_rebuild_same_type_img2img_batches/path_fence_flower_home_pipeline_v002/round169_path_fence_flower_home_godot_preview_1280x720_v002.png`；新增 Round169 报告；验证：Godot import、normalizer py_compile、preview export、`tests/test_v0239_visual_rebuild_blockout.gd`、`tests/headless_runner.gd` 均通过；本轮不接 runtime、不改 ThemeProfile / AssetResolver、不授予 `art_target_locked` / `runtime_visual_match` / `final_approved`；无新增已验证教训，`lessons.md` 不更新 |
| 2026-06-08 | Round 170-171 agent-based asset proof / Home-area preview | V02-VISUALREBUILD-002 composition-friendly proof-only asset generation | 调用 agents 生成 Round170 terrain / fence / flower / water / building packs 与 Round171 actors / ground-region packs；Round171 ground-region v001 确认能降低 full-screen `64x64` tile griddy look，但暴露大块地面 chroma 边缘残留；新增 v002 despill / alpha-edge repair normalizer，产出 `assets/art/visual_rebuild/round171/ground_regions/*_v002.*` 和 `docs/collaboration/round171_visual_rebuild_asset_generation/round171_home_area_godot_preview_1280x720_v002.png`；新增 / 收紧 `scripts/tools/audit_visual_candidate_manifests.py` locality gate，并将 Round169 / Round170 manifest 主引用与 normalized cell PNG 落到 asset tree；验证：ground-region normalizer py_compile、Round171 Godot proof export、Round169-Round171 manifest audit `17/17 pass`、proof-only summary 更新通过；本轮不接 runtime、不改 ThemeProfile / AssetResolver、不授予 `art_target_locked` / `runtime_visual_match`；新增 `LESSON-020` |
| 2026-06-08 | Round 172 agent-based town / home / A-Z prop proof | V02-VISUALREBUILD-002 proof-only prop and memory-palace anchor generation | 调用 agents 生成 `assets/art/visual_rebuild/round172/town_props/` 8 个小镇生活小物、`home_yard_props/` 8 个家园庭院小物、`anchor_props_a_m/` 13 个 A-M prop-first anchor 候选、`anchor_props_n_z/` 13 个 N-Z prop-first anchor 候选；新增 `docs/collaboration/round172_visual_rebuild_asset_generation/Round172_asset_generation_summary.md` 和 `tests/capture_round172_props_preview.gd`，导出 `round172_props_godot_preview_1280x720_v001.png`；Round172 manifest audit `4/4 pass`，A-Z 26 letters / stable `anchor_id` / `core_word` 对照 `data/maps/world_map.json` 无错误；本轮不接 runtime、不改 ThemeProfile / AssetResolver、不授予 `art_target_locked` / `runtime_visual_match`；无新增已验证教训，`lessons.md` 仅补轮次复盘 |
| 2026-06-08 | Round 173 agent-based furniture / portrait / UI / item proof | V02-VISUALREBUILD-002 proof-only inventory and UI asset generation | 调用 agents 生成 `assets/art/visual_rebuild/round173/furniture/` 8 个家具候选、`portraits_status/` 8 个角色 / Sunny 头像状态候选、`ui_icons/` 10 个 UI / 状态图标候选、`resources_shop_items/` 10 个资源 / 商店物品候选；新增 `docs/collaboration/round173_visual_rebuild_asset_generation/Round173_inventory_ui_asset_generation_summary.md` 和 `tests/capture_round173_inventory_ui_preview.gd`，导出 `round173_inventory_ui_godot_preview_1280x720_v001.png`；Round173 manifest audit `4/4 pass`；风险记录：`sunny_waiting` 有身份一致性风险，UI icons 仍需真实 Apple-like glass HUD 背景复核；本轮不接 runtime、不改 ThemeProfile / AssetResolver、不授予 `art_target_locked` / `runtime_visual_match`；无新增已验证教训，`lessons.md` 仅补轮次复盘 |
| 2026-06-08 | Round 174 agent-based NPC / landmark / weather proof | V02-VISUALREBUILD-002 proof-only character, place prefab, and weather FX asset generation | 调用 agents 生成 `assets/art/visual_rebuild/round174/npc_fullbody/` 8 个全身角色 / companion 站立候选、`place_prefabs/` 8 个 landmark / place prefab 候选、`weather_fx/` 8 个天气 / 环境 FX 候选；新增 `docs/collaboration/round174_visual_rebuild_asset_generation/Round174_character_place_weather_asset_generation_summary.md` 和 `tests/capture_round174_character_place_weather_preview.gd`，导出 `round174_character_place_weather_godot_preview_1280x720_v001.png`；Round169-Round174 manifest audit `28/28 pass`；首次 proof 暴露 NPC 深色矩形背景块，已重生成 chroma raw 并增加 `visual_background_block_count` gate；本轮不接 runtime、不改 ThemeProfile / AssetResolver、不授予 `art_target_locked` / `runtime_visual_match`；新增 `LESSON-021` |
| 2026-06-08 | Round 175 agent-based motion / interior / UI feedback proof | V02-VISUALREBUILD-002 proof-only motion, home interior, and interaction UI asset generation | 调用 agents 生成 `assets/art/visual_rebuild/round175/motion_sheets/` 6 个 4-direction walk motion sheet 候选、`home_interior_shells/` 8 个小屋室内 shell / region 候选、`interaction_ui_feedback/` 10 个交互反馈 / glass UI 候选；新增 `docs/collaboration/round175_visual_rebuild_asset_generation/Round175_motion_interior_ui_asset_generation_summary.md` 和 `tests/capture_round175_motion_interior_ui_preview.gd`，导出 `round175_motion_interior_ui_godot_preview_1280x720_v001.png`；扩展 `scripts/tools/audit_visual_candidate_manifests.py` 支持 path-like `main_ref` / `source_ref` 检查，Round169-Round175 manifest audit `31/31 pass`；本轮不接 runtime、不改 ThemeProfile / AssetResolver、不授予 `art_target_locked` / `runtime_visual_match`；新增 `LESSON-022` |
| 2026-06-08 | Round 176 agent-based place / album / seasonal proof | V02-VISUALREBUILD-002 proof-only place interior, album-card UI, and seasonal decor asset generation | 调用 agents 生成 `assets/art/visual_rebuild/round176/place_interior_shells/` 8 个 shop / school / bookshop / bus stop interior / front-area shell 候选、`album_card_ui/` 10 个相册 / 收藏卡 UI 候选、`seasonal_event_decor/` 10 个季节 / 活动装饰候选；新增 `docs/collaboration/round176_visual_rebuild_asset_generation/Round176_place_album_seasonal_asset_generation_summary.md` 和 `tests/capture_round176_place_album_seasonal_preview.gd`，导出 `round176_place_album_seasonal_godot_preview_1280x720_v001.png`；Round169-Round176 manifest audit `34/34 pass`；本轮不接 runtime、不改 ThemeProfile / AssetResolver、不授予 `art_target_locked` / `runtime_visual_match`；无新增已验证教训，`lessons.md` 仅补轮次复盘 |
| 2026-06-08 | Round 177 agent-based resource / feedback / transition proof | V02-VISUALREBUILD-002 proof-only world resource node, NPC feedback, and transition prop generation | 调用 agents 生成 `assets/art/visual_rebuild/round177/world_resource_nodes/` 10 个资源节点候选、`npc_feedback_emotes/` 10 个 NPC overhead feedback / emote 候选、`transport_transition_props/` 10 个交通 / 转场小物候选；新增 `tests/capture_round177_resources_feedback_transition_preview.gd` 并导出 `docs/collaboration/round177_visual_rebuild_asset_generation/round177_resources_feedback_transition_godot_preview_1280x720_v001.png`；完成 combined proof 目检：非空、三组区域对齐、未见明显深色背景块或 misalignment；Round169-Round177 manifest audit `37/37 pass`、missing path `0`、locality violation `0`；本轮不接 runtime、不改 ThemeProfile / AssetResolver、不授予 `art_target_locked` / `runtime_visual_match`；无新增已验证教训，`lessons.md` 不更新 |
| 2026-06-08 | Round 178 agent-based companion / marker / map-edge proof | V02-VISUALREBUILD-002 proof-only remaining visual asset gap generation | 调用 agents 生成 `assets/art/visual_rebuild/round178/companion_care_props/` 10 个 companion-care prop 候选、`gentle_quest_markers/` 10 个 gentle interaction marker 候选、`map_edge_nature_transitions/` 10 个 384x192 map-edge / nature transition 候选；新增 `tests/capture_round178_remaining_asset_gap_preview.gd` 并导出 `docs/collaboration/round178_visual_rebuild_asset_generation/round178_remaining_asset_gap_godot_preview_1280x720_v001.png`；combined proof 目检：非空、三组区域对齐、未见明显文字 / hard grid / dark block / misalignment；Round169-Round178 manifest audit `40/40 pass`、missing path `0`、locality violation `0`；本轮不接 runtime、不改 ThemeProfile / AssetResolver / data、不授予 `art_target_locked` / `runtime_visual_match`；无新增已验证教训，`lessons.md` 不更新 |
| 2026-06-08 | Round 179 agent-based home / daily / A-Z vignette proof | V02-VISUALREBUILD-002 proof-only home decor, daily prop, and A-Z memory-story vignette generation | 调用 agents 生成 `assets/art/visual_rebuild/round179/home_decor_surfaces/` 12 个 home decor surface 候选、`school_shop_daily_props/` 12 个 school / shop / bus stop daily prop 候选、`az_story_vignettes_a_m/` 13 个 A-M memory-story vignette 候选、`az_story_vignettes_n_z/` 13 个 N-Z memory-story vignette 候选；修正 N-Z manifest / generator 从 card IDs 改为 `data/maps/world_map.json` 稳定 `anchor_id` / `core_word` 校验；新增 `tests/capture_round179_home_daily_az_preview.gd` 并导出 `docs/collaboration/round179_visual_rebuild_asset_generation/round179_home_daily_az_godot_preview_1280x720_v001.png`；combined proof 目检：非空、四组区域对齐，未见明显背景块或 misalignment；A-Z coverage audit 确认 26/26 letter / anchor_id / core_word 无 missing / extra / mismatch；Round169-Round179 manifest audit `44/44 pass`、missing path `0`、locality violation `0`；本轮不接 runtime、不改 ThemeProfile / AssetResolver / data、不授予 `art_target_locked` / `runtime_visual_match`；无新增已验证教训，`lessons.md` 不更新 |
| 2026-06-08 | Round 180 mainline visual layout target | V02-VISUALREBUILD-002 Godot 可执行白盒目标与 visual layout contract 候选交付 | 停止继续扩展 proof-only asset warehouse，回到主线 `target 1280 frame -> visual layout contract`；新增 `docs/collaboration/round180_v0239_mainline_visual_layout_target/visual_layout_contract_v001.json`，覆盖 1280 viewport、home-centered camera、layer order、Home / School / Shop / water / A-Z subset / player / Sunny / five-button glass UI visual nodes、forbidden full-map / visible-grid / StoryBatch blocked 状态；新增 `tests/capture_round180_v0239_visual_layout_target.gd` 从合同读取并导出 `round180_v0239_visual_layout_target_1280x720_v001.png`；新增 `tests/test_v0239_visual_layout_target.gd` 并注册进 `tests/headless_runner.gd`；验证：capture、focused visual layout target、focused blockout、full headless runner、Godot 启动和 `file` 尺寸检查通过；本轮状态为 `target_frame_candidate_for_pm_user_review`，不接 runtime、不改 ThemeProfile / AssetResolver / data、不授予 `art_target_locked` / `runtime_visual_match`，不解锁 V02-VISUALREBUILD-004；复用 `LESSON-003`，无新增 lesson 编号 |
| 2026-06-08 | Round 181 V02.39 mainline closeout | V02-VISUALREBUILD-002/004/005/006 剩余主线任务收口与 StoryBatch 解锁 | 按用户“完成剩下所有任务”指令，将 Round180 Godot 白盒目标推进为 `art_target_locked_whitebox_by_user_instruction`，锁定范围限于 whitebox composition / runtime static-frame match，不等于最终 bitmap art approval；新增 `docs/collaboration/round181_v0239_mainline_closeout/unified_environment_composition_kit_v001.json`、`runtime_visual_match_report_v001.json`、`art_direction_gate_v001.json` 和 `round181_v0239_runtime_match_side_by_side_1280x720_v001.png`；新增 `tests/capture_round181_v0239_runtime_match_gate.gd`、`tests/test_v0239_visual_rebuild_mainline_gate.gd` 并注册 full runner；V02-VISUALREBUILD-002/004/005/006 完成，StoryBatch-004 阻塞解除并通过既有 runtime smoke，StoryBatch-005 转 Ready；验证：Round181 capture、mainline gate、visual layout target、blockout、StoryBatch-004 smoke、Godot headless 启动、full headless runner 和 `git diff --check` 通过；无新增已验证教训，`lessons.md` 不更新 |
| 2026-06-08 | Round 182 StoryBatch-005 proof / decision closeout | V02-STORYBATCH-005 1280 proof、approved 判定与阶段收口 | 新增 `tests/capture_round182_storybatch005_rc.gd`，通过真实 Main / story prop 交互后用 headless-safe Image proof 输出 7 张第二批 1280x720 PNG；新增 `docs/collaboration/round182_storybatch005_rc/storybatch005_approval_gate_v001.json` 和 `docs/collaboration/Round182_V02.37_STORYBATCH-005_1280ProofApprovedDecisionCloseout.md`，逐项判定第二批 C/D/G/K/O/S/W story props 为 `runtime_visual_match`，第一批四项维持 `functional_pass`，final bitmap art approval 递延；ThemeProfile 七条 Round153 story prop acceptance 保持 `production` 并将 `viewport_evidence` 指向 Round182 proof；新增 `tests/test_v0237_storybatch005_approval_gate.gd` 并注册 full runner；验证：capture check-only / capture、focused StoryBatch-005、StoryBatch-003、StoryBatch-004、PNG 尺寸、full runner、Godot headless 启动和 `git diff --check` 通过；无新增已验证教训，`lessons.md` 不更新 |
| 2026-06-08 | Round 183 runtime bitmap promotion | V02-VISUALREPLACE-001 visual rebuild 资产运行时优先替换 | 按用户“先把这些资产全部进行替换，不符合要求的再修改”指令，解除 Round169-Round179 仅 proof-only 的本轮 runtime 边界；`data/themes/theme_sunshine_town_placeholder.json` 接入 125 个 visual rebuild runtime resolver mappings，覆盖第一屏 `v0239_*` 以及现有 place / character / furniture / UI / anchor / story prop 映射，并保留 160 条 promoted acceptance 记录；`scripts/main.gd` 将 TownStage 第一屏实际使用的 `v0239_*` texture keys 绑定到 ThemeProfile / AssetResolver，成功解析时使用 PNG，失败时才 fallback 到程序化纹理；更新 V02.39 / AssetResolver / StoryBatch / full runner 断言，从 proof-only 或旧 production-only 改为 runtime-promoted mapping 与 acceptance path 一致；新增 `docs/collaboration/Round183_V02.39_VISUALREPLACE-001_RuntimeBitmapPromotion.md`；验证：ThemeProfile JSON、visual rebuild 路径审计、AssetResolver focused、StoryBatch focused、V02.39 blockout focused、V02.39 mainline gate、playable focused、full headless runner、Godot headless 启动、`git diff --check` 和持久 1280 proof 导出均通过；无新增已验证教训，`lessons.md` 仅补轮次复盘 |
